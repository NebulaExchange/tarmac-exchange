...
export const TradeWidgetNebula = ({
  onConnect,
...
function TradeWidgetWrapped({
  ...
  const needsAllowance =
    originToken &&
    quoteData &&
    quoteData.quoteSource === QuoteSource.COWSWAP &&
    !originToken.isNative &&
    !!(!allowance || allowance < quoteData.quote.sellAmountToSign);

  const isTransfer = quoteData && quoteData.quoteSource === QuoteSource.NEARINTENTS;
  ...
  const {
    ...
    onStart: (hash: string) => {
      addRecentTransaction?.({
        hash,
        description: t`Transferring ${formatBigInt(debouncedOriginAmount, {
          locale,
          unit: originToken ? getTokenDecimals(originToken, chainId) : 18
        })} ${originToken?.symbol ?? ''}`
      });
    ...
    onError: (error: Error, hash: string) => {
      onNotification?.({
        ...
        description: t`We could not transfer your tokens.`,
        status: TxStatus.ERROR
      });
      setTxStatus(TxStatus.ERROR);
      // mutateAllowance();
    ...
    enabled:
      widgetState.action === TradeAction.TRANSFER &&
      quoteData?.quoteSource === QuoteSource.NEARINTENTS &&
      !!originToken
  });
  ...
  const prepareError = approvePrepareError || ethTradePrepareError || transferPrepareError;
  ...
  useEffect(() => {
    if (widgetState.flow === TradeFlow.TRADE && widgetState.screen === TradeScreen.ACTION) {
      setWidgetState((prev: WidgetState) => ({
        ...prev,
        action:
          needsAllowance && !allowanceLoading
            ? TradeAction.APPROVE
            : isTransfer
  ...
  }, [widgetState.flow, widgetState.screen, needsAllowance, isTransfer, allowanceLoading]);
    ...
    setIsDisabled(
      isConnectedAndEnabled &&
        !!(
          ...
          (widgetState.action === TradeAction.TRANSFER && transferDisabled)
        )
    );
  }, [
    isQuoteLoading,
